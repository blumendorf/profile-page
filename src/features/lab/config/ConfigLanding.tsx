...
import { AlertTriangle, CheckCircle, Download, Zap, ArrowLeft, Settings2, Info, FileJson } from 'lucide-react';
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-text-muted text-lg leading-relaxed">
            AI generates a JSON UI specification that renders via React components.
            A middle ground between full HTML generation and simple config tweaks.
          </p>
        </div>

        {/* How it works */}
        <div className="bg-page-elevated rounded-lg p-6 mb-8 border border-border-subtle">
          <h3 className="font-mono text-sm text-amber-500 mb-4">How it works</h3>
          <ul className="text-sm text-text-muted space-y-2">
            <li>1. You describe a style (e.g., "terminal hacker", "warm corporate")</li>
            <li>2. AI generates a JSON structure describing the UI</li>
            <li>3. A React renderer interprets the JSON into components</li>
          </ul>
        ...
        {/* Google Generative UI inspiration */}
        <div className="bg-page-elevated rounded-lg p-6 mb-8 border border-border-subtle">
          <div className="flex items-start gap-3">
            <FileJson className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
            <div>
              <h3 className="font-mono text-sm text-amber-500 mb-2">Inspired by Generative UI</h3>
              <p className="text-sm text-text-muted leading-relaxed">
                Based on Google Research's{' '}
                <a
                ...
                , which shows LLMs can effectively generate custom UIs. Our approach uses a
                JSON UI Language that's easier for small local models to produce reliably.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Actions */}
        <Link
          to={canStart ? "/lab/config/playground" : "#"}
          ...
          className={`w-full py-4 px-6 rounded-lg font-semibold flex items-center justify-center gap-3 transition-colors
            ${canStart
              ? 'bg-amber-500 text-bg-page hover:bg-amber-400'
