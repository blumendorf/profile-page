...
export class TransformersEngine implements LLMEngine {
  ...
  async generate(prompt: string, config?: GenerationConfig, onToken?: TokenCallback): Promise<string> {
    ...
    console.log(`[transformers-engine] Config: max_new_tokens=${maxTokens}, temperature=${temperature}, top_p=${topP}, repetition_penalty=${repetitionPenalty}, streaming=${!!onToken}`);
