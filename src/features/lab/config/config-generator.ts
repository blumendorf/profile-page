...
JSON:`;
  ...
  try {
    const response = await webllmEngine.generate(prompt, { maxTokens: 200 });
    ...
    let parsed: Partial<UIConfig>;
    try {
      parsed = JSON.parse(jsonStr) as Partial<UIConfig>;
    } catch (parseErr) {
