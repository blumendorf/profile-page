...
The [Salesforce Lightning](https://www.lightningdesignsystem.com/) team called their token system the [Single Source of Truth](https://medium.com/salesforce-ux/living-design-system-3ab1f2280ef7) and stored it as JSON files. [Sönke Rohde's](https://medium.com/@sonkerohde) line, often quoted: *"It's basically a set of JSON files which contain name-value pairs describing our design tokens."* That was 2017. The pattern has held for nine years across [Material](https://m3.material.io/), [Carbon](https://carbondesignsystem.com/), [Polaris](https://polaris.shopify.com/), [SAP Fiori](https://experience.sap.com/fiori-design/), and most modern systems. The reason it has held is that it works, and the reason it works is mostly boring.
...
In a human-only team, the constraint reading is enforced by code review. In a 2026 team where an agent generates components, the constraint reading needs to be visible to the agent before the code is generated. That is what [Stitch's DESIGN.md format](https://github.com/google-labs-code/design.md) is doing with its YAML front-matter: not exporting tokens for the runtime, but declaring tokens as a constraint surface for whatever generator reads them.
...
A **headless primitive layer** for accessibility and behaviour: [Radix UI](https://www.radix-ui.com/), [React Aria](https://react-spectrum.adobe.com/react-aria/), [Headless UI](https://headlessui.com/). These handle the hard parts (focus management, keyboard navigation, ARIA roles) without imposing visual design. They're maintained by people who've thought about accessibility for longer than your team will.

A **styled component library** that consumes design tokens and produces visual components: [shadcn/ui](https://ui.shadcn.com/), [MUI](https://mui.com/), [Chakra](https://chakra-ui.com/). This is where the system's visual language lives. shadcn/ui's pattern of copying components into your repo (rather than installing as a dependency) is increasingly the default in 2026 because it lets agents read and modify the styled layer directly.
...
A **page or template layer** that arranges composition layers into routes. In a [Next.js](https://nextjs.org/) or [Astro](https://astro.build/) setup this maps to files; in older setups it's a more diffuse concept.
