...
export class WebLLMEngine implements LLMEngine {
  ...
  async generate(prompt: string, config?: GenerationConfig, onToken?: TokenCallback): Promise<string> {
    ...
    // Reset chat to avoid VectorInt binding errors
    ...
    console.log(`[webllm-engine] Config: max_tokens=${maxTokens}, temperature=${temperature}, top_p=${topP}, stop=${stop?.join(',')}, streaming=${!!onToken}`);
