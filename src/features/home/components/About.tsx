...
import { usePersona, type Persona } from '@/features/shared';
...
// Type for pillar data from site data
interface Pillar {
  icon: string;
  title: string;
  description: string;
...
const AboutContent = ({ persona }: AboutContentProps) => {
  ...
  const aboutVariants = (siteData).aboutVariants;
  const variantData = aboutVariants?.[persona];
  const paragraphs: string[] = variantData?.paragraphs ?? siteData.about.paragraphs;
  const pillars: Pillar[] = variantData?.pillars ?? siteData.about.pillars;
