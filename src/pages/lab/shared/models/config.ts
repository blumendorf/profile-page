/**
 * Model configuration with full metadata for UI display
 */
 ...
 */
export function getModelsByBackend(backend: 'transformers' | 'webllm'): ModelConfig[] {
  return AVAILABLE_MODELS.filter((m) => m.backend === backend);
}
