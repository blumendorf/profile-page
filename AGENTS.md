...
- **MCP config:** the canonical file is [`.agents/mcp.json`](.agents/mcp.json). Cursor reads from [`.cursor/mcp.json`](.cursor/mcp.json) and Claude Code from [`.mcp.json`](.mcp.json); both are symlinks pointing at the canonical file. All three paths are committed. If a server entry contains a machine-specific absolute path, prefer a launcher on `PATH` or document the assumed install location in this file.
